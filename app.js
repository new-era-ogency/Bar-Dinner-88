...
const TRANSLATIONS = {
  bg: {
    ...
    "trust.kicker": "Доверие на булеварда",
    "trust.title": "Добрите часове се издават първи в уикендите на Витоша",
    "trust.stat1": "Рейтинг от гостите",
    "trust.stat2": "Обслужени куверти",
    "trust.stat3": "Избор за булеварда",
    "trust.note":
      "Местни, хотелски гости и сомелиери в транзит — залата се пълни с хора, на които им пука как се чувства вечерта, не само как се снима.",
    "booking.urgency": "Премиум часовете изчезват първи — запазете си своя.",
    "sticky.line": "Тази вечер на Витоша — дръжим маса за вас.",
    "sticky.cta": "Резервирай",
  ...
  en: {
    ...
    "trust.kicker": "Trusted on the boulevard",
    "trust.title": "Prime tables disappear fast on Vitosha weekends",
    "trust.stat1": "Guest rating",
    "trust.stat2": "Covers served",
    "trust.stat3": "Boulevard pick",
    "trust.note":
      "Locals, hotel guests, and sommeliers in transit — the room fills with people who care how a night feels, not just how it photographs.",
    "booking.urgency": "Prime times book out first — lock yours now.",
    "sticky.line": "Tonight on Vitosha — we hold a table for you.",
    "sticky.cta": "Book now",
...
function initScrollRevealFallback(nodeList) {
...
function initGsapScroll() {
  const nodes = document.querySelectorAll("[data-gsap-scroll]");
  if (!nodes.length) return;
  ...
  if (typeof gsap === "undefined" || typeof ScrollTrigger === "undefined") {
    initScrollRevealFallback(nodes);
    return;
  }
...
function initHeroCinematic() {
  ...
  try {
    ...
    if (img) {
      gsap.set(img, { scale: 1.12, transformOrigin: "50% 42%" });
      tl.to(img, { scale: 1, duration: 2.35, ease: "power2.inOut" }, 0);
    }
    if (lines.length) {
      tl.from(
        lines,
        { autoAlpha: 0, y: 48, duration: 0.95, stagger: 0.1, ease: "power3.out" },
        0.12
      );
    }
  } catch {
...
function initPremiumHovers() {
  if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
  if (window.matchMedia("(pointer: coarse)").matches) return;
  if (typeof gsap === "undefined") return;

  document.querySelectorAll("[data-premium-hover]").forEach((root) => {
    const img = root.querySelector("img");
    const isGallery = Boolean(img && img.classList.contains("gallery-parallax-img"));

    root.addEventListener("pointerenter", () => {
      gsap.to(root, { y: -8, duration: 0.48, ease: "power2.out" });
      if (img && !isGallery) {
        gsap.to(img, { scale: 1.06, duration: 0.75, ease: "power2.out" });
      } else if (img && isGallery) {
        gsap.to(img, { filter: "brightness(1.12)", duration: 0.5, ease: "power2.out" });
      }
    });

    root.addEventListener("pointerleave", () => {
      gsap.to(root, { y: 0, duration: 0.65, ease: "power3.out" });
      if (img && !isGallery) {
        gsap.to(img, { scale: 1, duration: 0.75, ease: "power3.out" });
      } else if (img && isGallery) {
        gsap.to(img, { filter: "brightness(1)", duration: 0.55, ease: "power2.out" });
      }
    });
  });
}

function initStickyBarMotion() {
  if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
  if (typeof gsap === "undefined") return;

  const bar = document.querySelector("#sticky-book-cta [data-sticky-glow]");
  if (!bar) return;
  ...
  try {
    gsap.fromTo(
      ...
      {
        boxShadow: "0 0 32px -6px rgba(212,175,55,0.38)",
        duration: 1.55,
        repeat: -1,
        yoyo: true,
        ease: "sine.inOut",
...
/** Hide bottom booking strip while the full Tablein section is on screen (less clutter on mobile). */
function initStickyHideInBooking() {
  const sticky = document.getElementById("sticky-book-cta");
  ...
  const show = () => {
    ...
    if (typeof gsap !== "undefined") {
      gsap.to(sticky, { autoAlpha: 1, y: 0, duration: 0.35, ease: "power2.out" });
    } else {
  ...
  const hide = () => {
    ...
    if (typeof gsap !== "undefined") {
      gsap.to(sticky, { autoAlpha: 0, y: 14, duration: 0.35, ease: "power2.in" });
    } else {
      sticky.style.opacity = "0";
      sticky.style.visibility = "hidden";
      sticky.style.transform = "translateY(14px)";
    }
  };
  ...
  if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
    const setStickyVisible = (visible) => {
      if (visible) {
        ...
        sticky.style.opacity = "0";
        sticky.style.visibility = "hidden";
        sticky.style.transform = "translateY(12px)";
      }
    };
